...
if (strlen($_SESSION['alogin']) == 0) {
	...
	if (isset($_POST['submit'])) {
		...
		if ($_FILES["productimage1"]["error"] > 0) {
			$_SESSION['delmsg'] = "Error al cargar imagen";
		} else {
			$formats = array("image/jpg", "image/jpeg", "image/png");
			if (in_array($_FILES["productimage1"]["type"], $formats)) {
				$dir = "product_images/" . $productid . "/";
				$image = $dir . $_FILES["productimage1"]["name"];
				if (!file_exists($dir)) {
					mkdir($dir);
				}

				if (!file_exists($image)) {
					$resultado = @move_uploaded_file($_FILES["productimage1"]["tmp_name"], $image);
					if ($resultado) {
						chmod($image, 0766);
					...
					$_SESSION['delmsg'] = "La imgen insgresada ya existe";
				}
			} else {
				$_SESSION['delmsg'] = "Formato de imagen invalido";
			}
		}
	}
							...
							<div class="module">
								...
								<div class="module-body">
									...
									<form class="form-horizontal row-fluid" name="insertproduct" method="post" enctype="multipart/form-data">
										...
										<div class="control-group">
											<label class="control-label" for="basicinput">Imagen de producto</label>
											<div class="controls">
